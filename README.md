...
- **Scope**: 6 live experiment cards (EXP-005 Upadacitinib truthful-null, EXP-028 honesty test, EXP-031 OOD disagreement, EXP-032 adaptive gate, EXP-033 pipeline-level, EXP-034 path separation) + a foundational-iteration archive (EXP-001~030, real metrics/reports surfaced via the inspector). Pipeline-reliability heuristics only — not clinical efficacy. The public surface now distinguishes early manual-assisted pipeline controls (EXP-005), tiny fallback-gated honesty pilots (EXP-028), observer-only disagreement signals (EXP-031), legacy parity anchors (EXP-032), current-repro governance collapse (EXP-033), and accepted-anchor path separation (EXP-034) instead of flattening them into one success story.
