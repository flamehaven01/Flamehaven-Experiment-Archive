...
### 3.0 Registry-Driven Card Pattern (DI-BSC-001)
...
**Slot mechanism** (`slot: true`): Pre-registers an entry before its artifact files are uploaded to GitHub Pages. Effects:
- Card title is plain text (no viewer link)
- PENDING badge appears in the eyebrow
- Report/PDF/MD/JSON/Article buttons are disabled (`dl-btn-slot` class)
- Newsletter button shows disabled if `newsletterUrl: null`
- **Activation**: when files are uploaded, set `slot: false` (or remove the key) and set `newsletterUrl` / `articleUrl` to their URLs

**Newsletter button** (`newsletterUrl` field): New button type (`dl-btn-newsletter`, cyan `#06b6d4` accent). Renders when `newsletterUrl` key is **present** in the entry (even if null). Older entries without the key show no button. Pattern: `null` = button visible but disabled (slot), string URL = active link.

**Version bump rule**: After any change to `bsc-registry.js` or `bsc-renderers.js`, bump the `?v=X.Y.Z` cache-buster on all script tags in `index.html`. Failing to bump means browsers serve the old cached JS despite a fix being deployed. Current version: `1.22.0`.
