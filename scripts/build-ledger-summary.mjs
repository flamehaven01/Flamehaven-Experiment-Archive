...
const laneDefinitions = {
  ...
  toe: {
    ...
    title: 'EQA',
    status: 'active',
    summary: 'Equation-to-Artifact verification lane: deterministic reproduction of mathematical and governance research artifacts as runnable, inspectable records. Active named runs TOE-TEST-0052~0059 plus the 0001~0051 historical archive. Canonical live numbering is 0055 = AEFSO, 0056 = OpenAI Erdős reproduction, 0057 = QSOT Compiler v1.2.3 slop audit, 0058 = QSOT2 mathematical-consistency verifier, and 0059 = QSOT-Harness v2.1.2 (r1 hardening release) honest reconstruction (renumbered from 0058 in the 3-line QSOT split).',
  },
  rexsyn: {
    key: 'rexsyn',
    title: 'BAV',
...
function scanEqaRecords() {
  ...
  const targetDirs = ['toe-test-0055', 'toe-test-0056', 'toe-test-0057', 'toe-test-0058', 'toe-test-0059'];
  for (const dirName of targetDirs) {
    ...
    let id = dirName === 'toe-test-0055' ? 'toe-test-0055-aefso' : (dirName === 'toe-test-0056' ? 'toe-test-0056-openai-erdos-eq22' : (dirName === 'toe-test-0057' ? 'toe-test-0057-qsot-compiler' : (dirName === 'toe-test-0058' ? 'toe-test-0058-qsot2' : 'toe-test-0059-qsot-harness')));
    let title = `EQA: ${dirName.toUpperCase()} — ${manifestJson.title || ''}`;
    let auditDate = dirName === 'toe-test-0055' ? '2026-04-16' : (dirName === 'toe-test-0056' ? '2026-05-27' : (dirName === 'toe-test-0057' ? '2025-12-23' : (dirName === 'toe-test-0058' ? '2026-06-17' : '2026-06-15')));
    let summary = firstNonEmptyLine(analysisText) || manifestJson.working_hypothesis || '';
    let useScope = dirName === 'toe-test-0055' ? 'Optional backend layer only; not eligible for core promotion.' : (dirName === 'toe-test-0056' ? 'Fully published. Citable via Zenodo DOI.' : (dirName === 'toe-test-0057' ? 'Research reference and numerical consistency check only.' : (dirName === 'toe-test-0058' ? 'Mathematical-consistency verification line (supersedes 0057); sibling of 0059. Verified for model-output consistency only.' : 'Fully published honest reconstruction (renumbered from 0058 in the 3-line split). Citable via Zenodo DOI.')));
